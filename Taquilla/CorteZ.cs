...
namespace Taquilla
{
	public partial class CorteZ : Gtk.Dialog
	{
		...
		protected virtual void OnButtonOkClicked (object sender, System.EventArgs e)
		{
			...
			Tiquete += Imprimidor.Imprimir("**** CORTE Z ****",1);
			...
			double totalEventos = double.Parse(MySQL.Reader["totalEventos"].ToString());
			double TotalDelDia = totalCafeteria+totalEventos+totalJuegos;
			Tiquete += Imprimidor.Imprimir("Total:","$"+TotalDelDia.ToString("0.00"));
			Tiquete += Imprimidor.Imprimir("Total [JUEGO]","$"+totalJuegos.ToString("0.00"));
			Tiquete += Imprimidor.Imprimir("Total [EVENTOS]","$"+totalEventos.ToString("0.00"));
			Tiquete += Imprimidor.Imprimir("Total [CAFETERIA]","$"+totalCafeteria.ToString("0.00"));
			Tiquete += Imprimidor.Imprimir("T.Efectivo","$"+txtEfectivo.Text);
			Tiquete += Imprimidor.Imprimir("T.POS","$"+txtCredito.Text);
			Tiquete += Imprimidor.Imprimir("T.Cheque","$"+txtCheque.Text);
			Tiquete += Imprimidor.Imprimir("T.Compras","$"+txtTotalCompras.Text);
			...
			Tiquete += Imprimidor.Imprimir("Diferencia: "+TextoDiferencia,"$"+diferencia.ToString("0.00"));
			Tiquete += Imprimidor.Imprimir("Numero de compras:",txtNoCompras.Text);
			
			//Tiquete += Imprimidor.Imprimir("Transacciones:",MySQL.Reader["transacciones"].ToString());
			...
			while (MySQL.Reader.Read()) {
				Tiquete += Imprimidor.Imprimir("T:"+MySQL.Reader["descripcion"].ToString(), MySQL.Reader["cuenta"].ToString());
			}
			...
			while (MySQL.Reader.Read()) {
				Tiquete += Imprimidor.Imprimir("E:"+MySQL.Reader["ID_evento"].ToString(), MySQL.Reader["precio_evento"].ToString());
			}
			...
			while (MySQL.Reader.Read()) {
				Tiquete += Imprimidor.Imprimir("[_______] == " + MySQL.Reader["disponibles"].ToString(), MySQL.Reader["descripcion"].ToString());
			}	
			...
			while (MySQL.Reader.Read())	
			{
				Tiquete += Imprimidor.Imprimir(MySQL.Reader["hora"].ToString() + "," + MySQL.Reader["descripcion"].ToString() + "," + MySQL.Reader["cantidad"].ToString() + "," + "$" + MySQL.Reader["precio_total"].ToString(),1);
			}
			...
			Tiquete += Imprimidor.Imprimir("Hora", "No.", "Hora", "No.");
			...
			while (MySQL.Reader.Read()) {
				...
				Tiquete += Imprimidor.Imprimir(bCol1, bCol2, bCol3, bCol4);
			}
